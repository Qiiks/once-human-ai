                ...
                const builder = {
                    ...
                    then: async function(resolve, reject) {
                        try {
                            ...
                            const result = await pgClient.query(query, whereValues);
                    ...
                    upsert: async (data) => {
                        try {
                            ...
                            const query = `INSERT INTO ${table} (${keys.join(', ')}) VALUES (${placeholders}) ON CONFLICT (user_id, key) DO UPDATE SET ${updates} RETURNING *`;
                            const result = await pgClient.query(query, processedValues);
                            return { data: result.rows, error: null };
                        } catch (error) {
